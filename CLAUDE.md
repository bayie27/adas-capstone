...
- **`ai_engine/machine_profile.json` is machine-specific and gitignored**, like a TensorRT engine was. Produced by `uv run python ai_engine/calibrate.py`, which reports how many cameras the machine can carry at 10 and at 15 FPS. Absence is not an error — the engine falls back to a conservative default.
