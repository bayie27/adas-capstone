...
> `python`), `docs/archive/be_plan/01_CONTRACTS.md` §5 and §9, `docs/archive/be_audit/00_FINDINGS.md`.
...
- The paper's Technical Scope claims "HTTPS for standard API requests and WebSockets." Nothing in
  `docs/archive/be_plan/` implements it. **Owner decision: do real self-signed TLS**, which fixes F4 and makes
...
**Read `docs/archive/be_audit/DEMO_TOPOLOGY.md` first.** It carries the architecture rationale, the demo-day
...
1. **`docs/archive/be_audit/DEMO_TOPOLOGY.md` updated in place** with the commands you actually ran and any
