...
**For the session that runs this:** the two-machine LAN/TLS drill (`docs/archive/validation/LAN_DEMO_HANDOFF.md`) ran in full
...
`docs/archive/be_plan/EVIDENCE.md`'s "LAN-measured NFR-04" section (search for that heading) for the full account
...
`docs/archive/be_plan/EVIDENCE.md`'s NFR-04 row currently reads "not captured" for the real two-laptop LAN case.
...
3. `docs/archive/be_plan/EVIDENCE.md`, `docs/archive/be_audit/DEMO_TOPOLOGY.md` §11, and `docs/archive/be_audit/00_FINDINGS.md`'s
...
So Phase A and Phase B of the original drill (`docs/archive/validation/LAN_DEMO_HANDOFF.md`) are **already done** — don't
...
`docs/archive/be_plan/EVIDENCE.md`'s NFR-04 section; summary:
...
existing text in `docs/archive/be_plan/` or delete rows in `00_FINDINGS.md`):

1. **`docs/archive/be_plan/EVIDENCE.md`** — replace the "LAN-measured NFR-04 — attempted 2026-08-17, still not
   captured" section's content with the real figure and method, and update the summary table row.
2. **`docs/archive/be_audit/DEMO_TOPOLOGY.md` §11** — add a dated note under the existing 2026-08-17 log entry
   (don't delete or rewrite it) recording that the NFR-04 gap it left open is now closed, with the
   figure and method.
3. **`docs/archive/be_audit/00_FINDINGS.md`** — append one dated resolution-log line.
...
- **Don't print secrets** (`DEFAULT_ADMIN_PASSWORD`, `SECRET_KEY`, `INTERNAL_API_KEY`, `DSS_*`) into
  the transcript.
- **`docs/archive/be_plan/` is append-only.**
- Both machines are expected to stay in their current LAN/TLS-configured state until after the real
  ...
  Don't run the `docs/operations/LAN_SETUP.md` §11 revert steps as part of this task.
