...
- **`capacity.py` has two modes and the default runs the real engine.** Closed-loop mode publishes clips through MediaMTX, brings up real `CameraStream`s and measures the frame rate each camera _achieved_, so decode, thread contention and `run()`'s scheduler are inside the number; it needs `mediamtx` and `ffmpeg` on PATH, or `--source` aimed at a running server. `--mode inference` is the original batched-inference sweep, kept because it needs neither binary and because closed-loop uses it as the seed. Both figures land in the profile and `main.py` prefers the measured one. The sweep reads high — it benchmarks 720p with no decode while the clips are 1440p — and the printed gap between them is the point. Don't "fix" a 10 FPS capacity that comes out below the 15 FPS one by clamping it: that means the runs disagreed and neither should be quoted.
