...
[`docs/archive/be_audit/DEMO_TOPOLOGY.md`](../archive/be_audit/DEMO_TOPOLOGY.md) instead. This document assumes that decision
...
or set `ADAS_MEDIAMTX_DIR` in your environment and drop the flag. If neither is set, the launcher also
checks the repository and its parent for an extracted `mediamtx*.exe` release. `-CertDir` overrides the certificate
...
`start-sim.ps1` still requires `ffmpeg`, and it requires MediaMTX either on `PATH`, via
`-MediaMtxDir`/`ADAS_MEDIAMTX_DIR`, or in a repo-local/extracted release directory it can discover.
It reads `mediamtx.yml`; its `paths` entries define the channel names and local media inputs. Change
that YAML to swap clips or change the number of simulated channels; `start-sim.ps1` discovers the
configured inputs automatically.
...
- **Kill the AI engine** → every fed camera eventually reads `Unresponsive`, but **not necessarily
  ...
  Tracked as `docs/archive/be_audit/00_FINDINGS.md` F33.
...
| Cameras stay `Connected` for over a minute after the AI engine is killed, dashboard untouched               | No background sweep pushes the `Unresponsive` transition — it's computed only when the frontend fetches                     | Refresh the page, or navigate to a different tab; the underlying status is already correct, it just hasn't been pushed. `docs/archive/be_audit/00_FINDINGS.md` F33        |
| Console fills with `WebSocket connection to 'wss://localhost:5173/...' failed` / `[vite] failed to connect` | Vite's own HMR socket, not `/ws/alerts` — it targets `localhost`, which is the client itself when browsing via `adas.local` | Harmless for a demo; the real alerts WebSocket is unaffected. `docs/archive/be_audit/00_FINDINGS.md` F34                                                                  |
...
| `start-sim.ps1` exits immediately                                                                           | `mediamtx`/`ffmpeg` unavailable and MediaMTX was not discoverable                                                           | Install/add `ffmpeg`; pass `-MediaMtxDir`, set `ADAS_MEDIAMTX_DIR`, or put an extracted `mediamtx*.exe` release beside the repo                                           |
...
| Cameras stuck on `Reconnecting`                                                                             | No RTSP feed for that channel, or MediaMTX is not running                                                                   | Step 6.1. Check that the camera's channel exists in the `paths` section of `mediamtx.yml`; the database roster and simulated channel list are configurable independently. |
...
- [`docs/archive/be_audit/DEMO_TOPOLOGY.md`](../archive/be_audit/DEMO_TOPOLOGY.md) — why wired LAN, the production mapping,
  demo-day runbook, hazards, fallback ladder, and the questions a panel is likely to ask
- [`docs/archive/be_audit/A1_lan_tls_drill.md`](../archive/be_audit/A1_lan_tls_drill.md) — the engineering pack that
  implemented the TLS profile
- [`docs/archive/be_audit/00_FINDINGS.md`](../archive/be_audit/00_FINDINGS.md) — F4 (the Secure-cookie failure this setup
  fixes), F5, F20
