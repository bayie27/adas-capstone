...
| `/admin/detections` (Logs)    | `112:8438`                         | Full toolbar — search, date range, three dropdowns, `Export`; `Resolved` in `--color-success`, `Dismissed` in `--color-fg-muted`.                                                                                                                                                                                                                                          |
...
| Alert modals                  | `118:8948`, `129:9234`, `124:9186` | Banner colour per status — `--color-danger` unverified, `--color-warning` ongoing, neutral resolved; badge fill; metadata label treatment (`caption` + uppercase + `tracking-[0.08em]`); button pairing; **no actions on the resolved variant**.                                                                                                                           |
...
- **`Resolve Accident` is now `--color-primary`, not green.** §2.2 lists it
