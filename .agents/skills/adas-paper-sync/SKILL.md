...
For Sheet comments, run a native-anchor capability preflight. A `sheet_cell_range` or quoted `Cell/range ...` value is not a provider-valid anchor. If the connector cannot create and verify a non-empty native anchor tied to the intended range, record `No comment — provider-native Sheet anchor unavailable.`, mark the comment item blocked when it was approved, and do not call or retry the comment endpoint. Do not use a browser or UI fallback. Apply approved cell updates independently. If an incorrect, duplicate, or provisional comment was nevertheless created by a lower-level failure, delete it with the provider's comment-deletion operation and verify deletion; do not resolve it as a substitute.
