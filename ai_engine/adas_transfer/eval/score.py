...
Usage:
    prototype/.venv/Scripts/python.exe eval/score.py --events-dir runs/events
    prototype/.venv/Scripts/python.exe eval/score.py --events-dir runs/events --lead 2 --tail 10
"""
    ...
    with open(path, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            try:
                ...
                print(f"[warn] skipping malformed row for {clip!r}")
                continue
...
def main() -> None:
    ...
    ap.add_argument("--events-dir", required=True, help="dir of *.json written by detect/run.py")
    ...
    if not os.path.exists(args.labels):
        print(f"[error] no labels file at {args.labels}")
        print("[error] create it first — nothing can be scored without ground truth")
        return
    labels = load_labels(args.labels)
    files = sorted(glob.glob(os.path.join(args.events_dir, "*.json")))
    if not files:
        print(f"[error] no event files in {args.events_dir}")
        return
    ...
    for path in files:
        ...
        if wins is None:
            print(f"[warn] {clip}: no label row — treating whole clip as non-crash")
            wins = []
    ...
    print(f"{'clip':<34}{'crashes':>8}{'hit':>5}{'miss':>6}{'FP':>5}{'events':>8}{'dur':>8}")
    print("-" * 74)
    ...
    print(f"FP PER MINUTE   {fp_per_min:.2f}  (over {tot_clean_s/60:.1f} min of non-crash footage)")
    ...
    if mins < 10.0:
        ...
        print("[note] does not establish a deployment alert rate. Say so in any write-up.")
    else:
        print(f"\n[note] FP/min measured over {mins:.1f} min of non-crash footage. This is a real")
        print("[note] rate, but it is still one camera set on one day - not a deployment average.")
