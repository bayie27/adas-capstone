...
- **One accumulator per camera, reset on reconnect, resume and restart.** A fired region is retained forever and keeps absorbing detections at that location, so a shared instance goes permanently deaf there. `camera.py`'s `segment_id` increments at all three seams and `pipeline.py` resets on the change. Consume `segment_id` as an **equality check**, never as a delta or a count — `+= 1` is not atomic, and comparing deltas turns a lost increment into a missed reset.
