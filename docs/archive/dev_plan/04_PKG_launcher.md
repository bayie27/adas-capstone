...
> `scripts/start-sim.ps1` (the shape to follow) · `docs/archive/be_audit/DEMO_TOPOLOGY.md` §5 (the real demo-day
> bring-up order)
> **Size:** S. Four steps.
...
Then follow `docs/archive/be_audit/DEMO_TOPOLOGY.md`'s bring-up order: MediaMTX → backend → frontend → AI engine.
...
instructions below it as the manual fallback (`docs/archive/be_audit/DEMO_TOPOLOGY.md`'s fallback ladder depends
