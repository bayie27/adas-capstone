...
> **Prerequisite reading:** [`01_CONTRACTS.md`](01_CONTRACTS.md) §9, `docs/archive/be_decisions_review.md` D-008.
> **Size:** M. Seven steps.
...
- Protocol-level ping/pong is satisfied at the ASGI-server layer, not the application layer —
  **verified, not built** (`docs/archive/be_audit/A4_realtime_hardening.md` F6, 2026-08-10). `websockets 16.0`
  ...
  - `uv run uvicorn app.main:app --app-dir backend ...` (the direct-uvicorn TLS demo command,
    `docs/archive/be_audit/A1_lan_tls_drill.md` step 2) — this is the one launch path that **can** pin the
