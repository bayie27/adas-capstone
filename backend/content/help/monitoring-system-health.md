...
usage) show a single line. Some readings, CPU temperature in particular, may show an
"Unavailable" message noting it isn't reported on this host, if the server's hardware
doesn't expose that sensor; that's a hardware limitation, not an error.
