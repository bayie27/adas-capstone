...
- Add the P25 row to `docs/archive/be_plan/00_INDEX.md`'s status table. This document already lives at
  `docs/archive/be_plan/25_PKG_audit_target_labels.md`; commit it as `docs(planning):` if it is still
  untracked.
- No OpenAPI description changes — no route signature or response model changed.
- No `paper_sync` finding. See "Paper impact" below.
- Update `docs/archive/be_plan/EVIDENCE.md` only if that document quotes a PDF filter-summary header
