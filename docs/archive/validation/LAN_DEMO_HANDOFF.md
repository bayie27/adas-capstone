...
**For the session that runs this:** you are executing [`docs/operations/LAN_SETUP.md`](../../operations/LAN_SETUP.md) for real, on two
physical machines, and writing the results back into the repo. All of the research is already done and
is captured below — do not re-derive it. Read this file, read `docs/operations/LAN_SETUP.md`, then start at Phase A.
...
`docs/archive/be_audit/DEMO_TOPOLOGY.md` §10 and the A1 resolution log in `docs/archive/be_audit/00_FINDINGS.md` both record the
same gap: the A1 LAN/TLS drill ran on **one physical machine**, so every step needing a second
machine's hands was never executed. `docs/archive/be_plan/EVIDENCE.md` carries the consequence — the NFR-04 figure
...
7. The results are written back into `docs/operations/LAN_SETUP.md`, `docs/archive/be_audit/DEMO_TOPOLOGY.md`,
   `docs/archive/be_audit/00_FINDINGS.md` and `docs/archive/be_plan/EVIDENCE.md` (Phase E)
...
| `docs/operations/LAN_SETUP.md`                             | **The procedure you are executing.** Phases below map onto its steps.                    |
| `docs/archive/be_audit/DEMO_TOPOLOGY.md` §4–§8          | Addressing rationale, known hazards, the fallback ladder if something breaks             |
| `docs/archive/be_audit/A1_lan_tls_drill.md` steps 1–7   | The pack that built the TLS profile; step 6 is the certificate trust story               |
| `docs/archive/be_audit/00_FINDINGS.md` rows F4, F5, F20 | F4 is what this proves; F5 has a browser check still owed; F20 explains the outbox drain |

You do not need to read `docs/archive/be_plan/` beyond `EVIDENCE.md`'s NFR-04 section.
...
at 7.5 in `docs/operations/LAN_SETUP.md` means step 4 was done into the wrong store.
...
equivalent, with the rationale for every flag, is in `docs/operations/LAN_SETUP.md` §6 — read it if you need to run one
...
The owner drives the browser; you read server logs alongside and correlate. Follow `docs/operations/LAN_SETUP.md` §7,
...
**LAN NFR-04 measurement (optional, and only if honest).** `docs/archive/be_plan/EVIDENCE.md` wants a real
...
1. **`docs/operations/LAN_SETUP.md`** — correct any step that turned out wrong, and add anything that cost you time to
...
2. **`docs/archive/be_audit/DEMO_TOPOLOGY.md` §10** — replace "What is still owed on real hardware" with a dated
...
3. **`docs/archive/be_audit/00_FINDINGS.md`** — append one dated line to the resolution log. Update the **F5** row
...
4. **`docs/archive/be_plan/EVIDENCE.md`** — update the "LAN-measured NFR-04 — still owed" section with either the
...
Then walk `docs/operations/LAN_SETUP.md` §11 with the owner to shut down and revert both machines.
...
- **`docs/archive/be_plan/` is append-only** — add dated notes, never rewrite existing text.
...
- `docs(ops):` — `docs/operations/LAN_SETUP.md` updates
- `docs(audit):` — `docs/archive/be_audit/` and `docs/archive/be_plan/EVIDENCE.md` updates

`docs/operations/LAN_SETUP.md` and this file live at the repo root, so **they are scanned by `prettier --check .`** in
the `pnpm check` pre-push gate (`.prettierignore` excludes `docs/archive/be_plan/` and `docs/archive/be_audit/`, not the root).
After editing either, run:

```bash
pnpm exec prettier --write docs/operations/LAN_SETUP.md docs/archive/validation/LAN_DEMO_HANDOFF.md
```
