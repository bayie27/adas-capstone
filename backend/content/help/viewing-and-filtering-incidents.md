...
Filters combine with each other, not replace one another — narrowing by camera and then
adding a status filter looks for incidents matching both, not either.
...
— so filtering here is also how you scope a report before downloading it.
