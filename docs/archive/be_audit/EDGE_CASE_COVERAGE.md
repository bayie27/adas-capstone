...
Built 2026-08-11 by the A5 audit pack (`docs/archive/be_audit/A5_edge_cases.md`, Part 2), closing F12: P9's
...
status column, one row per case in `docs/archive/be_plan/14_EDGE_CASES.md`.
...
`docs/archive/be_audit/00_FINDINGS.md` F24–F31 for the findings that came out of doing this.
...
| 5.10 | `MAINTENANCE_HOUR_LOCAL` across a DST transition has defined behavior | P7, P18 | covered (backup half) / inapplicable (restart half) | P18 made `MAINTENANCE_HOUR_LOCAL` live on both halves. Backup half: `app.main` registers a real APScheduler `CronTrigger` with an explicit `timezone=REPORT_LOCAL_TIMEZONE` — `test_maintenance_schedule.py::TestDailyBackupCronTimezone` asserts the registered `daily_backup` job's trigger timezone is `Asia/Manila`, not UTC (the silent-8-hour-shift guard), and `::TestDailyBackupCronDstBehavior` asserts a `CronTrigger` built the same way keeps firing at local 03:00 across a real DST transition (`America/New_York`, since `Asia/Manila` has no DST to prove anything with). Restart half stays genuinely OS-level — a Windows Scheduled Task (`scripts/register-maintenance-task.ps1`, task `\ADAS\DailyRestart`) or the systemd timer in `deploy/systemd/adas-maintenance.timer` — documented in `test_maintenance.py::TestSchedulingEdgeCases::test_restart_scheduling_is_the_os_schedulers_responsibility` (skipped, with rationale) |
| 5.11 | Scheduler misfire after suspend/resume coalesces into one catch-up run | P4, P5, P7, P18 | covered | `test_scheduler.py::TestMisfireCoalescing::test_a_backlog_of_missed_fires_collapses_into_one_catch_up_run` — a real APScheduler instance with `next_run_time` rewound to simulate 5 missed fires, asserts exactly one coalesced execution. P18 adds a second, independent catch-up path for the backup specifically: `daily_backup_catch_up` (hourly) plus a lifespan-startup due-check, both exercised in `test_maintenance_schedule.py::TestRunDailyBackupIfDue`. The restart half stays out of Python's control (OS-scheduler-level), as already documented |
...
All recorded in `docs/archive/be_audit/00_FINDINGS.md`. F24–F30 are fixed; F31 is open (upstream library boundary):
