...
`docs/archive/be_plan/16_HEARTBEAT_VS_POLLING.md`: at 418 cameras the heartbeat is ~56 KB/s, 0.05% of a gigabit
...
| Docker | Rejected in `docs/archive/be_plan/13_WSL2_LINUX_PATH.md` — GPU passthrough means matching drivers and rebuilding TensorRT inside the container, plus RTSP across the container boundary. Large new failure surface, zero demo benefit. |
...
Per `docs/archive/be_plan/15_PKG_ai_engine_integration.md`: if it still says `Reconnecting`, nothing else matters
...
Executed per `docs/archive/validation/LAN_DEMO_HANDOFF.md`, with the owner physically present at both keyboards (server
...
- **Phase B (client)** — static IP `192.168.50.2/24`, `certs/adas-cert.pem` copied across (key
  ...
  equally-valid destination and is now documented as an alternative — see `docs/operations/LAN_SETUP.md` Step 3c),
...
- **The server's network profile flipped from Private back to Public mid-drill, with no IP change
  to trigger it** (`docs/operations/LAN_SETUP.md` Step 2 previously documented only the addressing-change trigger).
...
- **Cameras did not read `Unresponsive` within ~10s of the AI engine dying while the dashboard sat
  idle**, contradicting this document's own §4 script and `docs/operations/LAN_SETUP.md` §7.9. Root-caused to source:
...
- **An attempted precise LAN NFR-04 latency measurement was abandoned mid-attempt.** A console
  ...
  and was reverted via a hard reload. See `docs/archive/be_plan/EVIDENCE.md`'s NFR-04 section for why the figure
