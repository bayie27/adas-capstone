...
**8 cameras at 15 FPS, 12 at 10 FPS** on the GTX 1650, benchmarked contiguously over batches 1–16. `machine_profile.json` is written and gitignored; re-run `uv run --no-sync python ai_engine/capacity.py` on any other machine.
...
⚠️ `capacity.py` does **not** probe for it or build an engine — see "Still to do" below. It benchmarks the plain `.pt` weights, so the 8/12-camera capacity is a floor.
...
- **`measured_fps` is the DECODE rate, not the detection rate.** `camera.py:152` records it in the reader thread right after `cap.read()`, so it tracks the stream's native rate (16–31 observed live) and has no relationship to how often the AI actually infers on that camera. It is not surfaced in `frontend/src`, so no operator sees it, but it is on the camera record and in the API — and anyone comparing it against the paper's 10–15 FPS band (p.74, TC-AI-402) will draw the wrong conclusion in one direction or the other. Consider renaming to `decoded_fps`.
- **Roughly half of every decoded frame is discarded.** The reader thread decodes continuously (~30 fps for the sample clips) while the pipeline consumes one frame per tick (15 fps, or 10 over capacity), and `_update()` overwrites `_latest` between ticks. Taking the newest frame is deliberate and correct — buffer size is pinned to 1 because a stale frame is worse than a dropped one — but decoding frames destined to be overwritten is wasted CPU. The engine already has the fix for the other case: the paused branch (`camera.py:130-133`) calls `cap.grab()` without decoding, commented "saves CPU". Applying it to the active path would mean the reader threads knowing the tick schedule, which re-couples two loops that are decoupled on purpose — so this is a real design change, not a tweak. It matters most on the CPU-only path, where decode and inference compete for the same cores, and it inflates the decode contention the calibration spec's Phase A sets out to measure. Get the calibration numbers first; they will say whether decode binds on any machine worth caring about.
