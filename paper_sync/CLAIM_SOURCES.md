...
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Capacity, FPS, batch size                           | `docs/archive/be_plan/EVIDENCE.md` · `ai_engine/machine_profile.json` (gitignored, machine-specific) · `uv run python ai_engine/capacity.py --model <path>`    |
...
| Latency, query time, export speed                   | `docs/archive/be_plan/EVIDENCE.md` · `uv run pytest -m slow backend/tests/perf/ -s`                                                                            |
| Test-case status, owner, evidence type              | `docs/archive/be_plan/TRACEABILITY.md` — vocabulary is `pass` / `pending` / `blocked`; D-001 forbids a blank owner                                             |
| Manual drill results                                | `docs/archive/be_plan/MANUAL_TESTS.md` — dated results blocks, appended never overwritten                                                                      |
| Known defects and accepted gaps                     | `docs/archive/be_audit/00_FINDINGS.md` — a row leaves `open` only by fix or written accepted-gap rationale                                                     |
| Deployment reality, and what is _not_ claimed       | `docs/archive/be_audit/README.md` decision 4 · `docs/validation/test-execution-validation-plan.md` → _Deviations from the Production Target_                   |
| Model, toolchain, and library versions              | `docs/validation/test-execution-validation-plan.md` → _Software Version Manifest_ · `pyproject.toml`                                                           |
...
| Scheduled maintenance, backup, restart              | `docs/archive/be_plan/18_PKG_scheduled_maintenance.md` · `scripts/adas-maintenance.ps1` · `scripts/register-maintenance-task.ps1` · `backend/app/maintenance/` |
...
| Frozen technical contract                           | `docs/archive/be_plan/01_CONTRACTS.md`                                                                                                                         |
| Locked decisions                                    | `docs/archive/be_decisions_review.md` (D-001…D-012)                                                                                                            |
...
- **`final_paper_text.txt` does not exist.** `docs/archive/be_plan/00_INDEX.md` and `docs/archive/be_plan/TRACEABILITY.md` still reference it. The live Google Doc replaced it; those references are stale.
...
- **Every number in `docs/archive/be_plan/EVIDENCE.md` is demo-hardware-validated** (D-009), never a production-scale claim. Carry that qualifier into any replacement text that quotes one.
