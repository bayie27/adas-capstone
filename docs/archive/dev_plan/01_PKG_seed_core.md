...
- **Changing `perf`'s 100 000-row behaviour or its statistical distribution.**
  `backend/tests/perf/` asserts against it and `docs/archive/be_plan/10_PKG_migration_evidence.md` is the
...
3. **Both entry points take an optional `target_settings`.** `init_db(engine)` alone migrates the
   ...
   `alembic/env.py` and ignores the engine argument (F18 in `docs/archive/be_audit/00_FINDINGS.md`). Step 2's
