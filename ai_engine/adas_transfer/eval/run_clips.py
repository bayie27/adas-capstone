...
Each clip runs in its OWN process. The old prototype was silently corrupted once by Ultralytics
state leaking between videos - `model.predictor = None` does not reset it. Separate processes
make that class of bug impossible rather than merely unlikely.

Usage:
    prototype/.venv/Scripts/python.exe eval/run_clips.py --weights models/weights_v2/best.pt
    prototype/.venv/Scripts/python.exe eval/run_clips.py --weights models/weights_v1/last.pt \
...
SAMPLES = os.path.join(ROOT, "prototype", "samples")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run + score the detector on the Lipa clips.")
    ...
    if not args.skip_run:
        for i, clip in enumerate(clips, 1):
            ...
            print(f"\n[{i}/{len(clips)}] {clip}")
            cmd = [sys.executable, os.path.join(ROOT, "detect", "run.py"),
                   "--video", os.path.join(SAMPLES, clip),
    ...
    print("\n" + "=" * 74)
    subprocess.run([sys.executable, os.path.join(ROOT, "eval", "score.py"),
                    "--events-dir", args.events_dir,
