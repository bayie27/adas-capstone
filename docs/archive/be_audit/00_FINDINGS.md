...
`docs/archive/be_audit/A*.md` updates the rows it owns. A finding leaves `open` only by being fixed, or by
...
- `2026-08-10` — A1 executed. **F4 fixed**: real self-signed TLS end-to-end (backend via uvicorn
  ...
  (`ADAS_TLS_CERT_DIR` unset). `.prettierignore` gained a `docs/archive/be_audit/` entry (mirroring the existing
  `docs/archive/be_plan/` exclusion) since `pnpm check` otherwise fails on this pack's own working docs.
...
- `2026-08-10` — A4 executed.
  **F6 fixed (verify-and-document, as scoped — not a build job).** Confirmed live, not just by
  ...
  boot on `/healthz/live`. `docs/archive/be_plan/04_PKG_realtime.md` Step 5 amended to record all of this so a
...
- `2026-08-10` — A3 executed. **F3 fixed**: `GET /api/internal/cameras`, `PATCH
  ...
  clock-skew warning. **F19 added**: `docs/archive/be_plan/MANUAL_TESTS.md` and `08_PKG_backup_ops.md` still
  reference the now-deleted `seed_alerts_via_api.py` — owed to A6. `docs/archive/be_plan/01_CONTRACTS.md` §6 and
...
- `2026-08-11` — A5 executed (edge case closure, Part 1). **F23 added and fixed** while closing
  ...
  **F12 fixed / Part 2 complete**: `docs/archive/be_audit/EDGE_CASE_COVERAGE.md` built by walking all ten
...
- `2026-08-11` — Second A5 pass, on request: closed every remaining `partial`/`uncovered` row from
  `docs/archive/be_audit/EDGE_CASE_COVERAGE.md`, not just the seven materially-risky ones already numbered.
  ...
  **1.18**'s genuinely-parallel test found and fixed a real bug — `apply_observed()` had the same
  ...
  instruction.
- `2026-08-16` — P18 (`docs/archive/be_plan/18_PKG_scheduled_maintenance.md`) executed. **F22 fixed**:
...
- `2026-08-17` — The two-machine LAN/TLS drill A1 always owed finally ran, on real hardware, in a
  real browser on a second physical machine (`docs/archive/validation/LAN_DEMO_HANDOFF.md`). All six of its own
  ...
  **F32, F33, F34 added** — a spontaneous Private→Public network-profile revert with no addressing
  ...
  reliable number and was abandoned — see `docs/archive/be_plan/EVIDENCE.md`'s NFR-04 section for the full
  reasoning. `docs/operations/LAN_SETUP.md` and `DEMO_TOPOLOGY.md` §10 updated with what actually happened.
